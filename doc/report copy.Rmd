...
data_bins <- bins(23, data=data) # bins(30, data=data) or bins(23, data=data)
#data_bins <- data_bins[1:500,] #shorter example

test_idx <- sample(1:nrow(data_bins), round(nrow(data_bins)/5, 0))
train_idx <- setdiff(1:nrow(data_bins), test_idx)
...
```{r message=F}
load(file = "../output/mat_fac.RData") #? load before save
